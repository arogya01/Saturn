...
## Why This Matters
A package manager that resolves correctly but fetches carelessly is still unreliable. Real projects are installed repeatedly. Teams rebuild environments. deployment systems start from scratch. If `saturn` redownloads everything every time or cannot verify artifacts, it will not scale beyond a demo.
...
- Decide what errors should be surfaced when a fetch fails or integrity check fails.
